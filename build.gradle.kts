...
fun refreshConstants() = file("src/main/java/com/wuyr/hookworm/core/Constants.java").writeText(
    file("src/main/resource/template_constants").readText().format(moduleMainClass)
)

fun File.buildModule() {
    val task = rootProject.getTasksByName("assemble", true)
        .filter { it.project.name == "app" }.run { if (isEmpty()) null else get(0) }
